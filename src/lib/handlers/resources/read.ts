...
export const handleResourcesRead = async (
  ...
  if (!uri) {
    const errorResp = {
      jsonrpc: "2.0",
      id,
      error: { code: -32602, message: "Invalid params: expected uri" },
    } as MCPResponse;
    try {
      sink?.resourceReadFail?.({
        id,
        uri,
  ...
  const providers: Array<MCPResourceProvider<unknown>> = toolkits.flatMap((tk) =>
    tk.resources ?? []
  );
  ...
  if (provider) {
    ...
    if (!tk) {
      const errorResp = {
        jsonrpc: "2.0",
        id,
        error: { code: -32002, message: "Resource provider not found" },
      } as MCPResponse;
      try {
        sink?.resourceReadFail?.({
          id,
    ...
    const result: MCPResourceReadResult = resultPromise instanceof Promise
      ? await resultPromise
      : resultPromise;
    try {
      sink?.resourceReadSuccess?.({ id, uri });
    } catch { /* ignore sink errors */ }
    return { jsonrpc: "2.0", id, result };
  }

  const templates: Array<MCPResourceTemplateProvider<unknown>> = toolkits.flatMap((tk) =>
    tk.resourceTemplates ?? []
  );
