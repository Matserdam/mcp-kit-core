...
import { uriMatchesTemplate } from "../../resources/util.ts";
...
export const runFetch = async (
  ...
  if (!resId || resId.length === 0) {
    return {
      jsonrpc: "2.0",
      id,
      error: { code: -32602, message: "Invalid params: expected { id: string, uri?: string }" },
    };
  }
  ...
  if (!targetUri) {
    return {
      ...
      error: {
        code: -32602,
        message: "Invalid params: provide a resolvable uri or use id as a url",
      },
    };
  }
...
const readViaProviders = async (
  ...
  if (provider) {
    ...
    try {
      ...
      const linkFallback: MCPToolCallResult = {
        content: [{ type: "resource_link", name: provider.name, uri }],
      };
      if (!Array.isArray(contents) || contents.length === 0) {
        return { jsonrpc: "2.0", id, result: linkFallback };
      }
      ...
      return {
        jsonrpc: "2.0",
        id,
        result: { content: [{ type: "resource_link", name: provider.name, uri }] },
      };
    }
  }
