...
export const handleToolCall = async (
  ...
  if (!params) {
    const errorResp = {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Params missing" },
    } as MCPResponse;
    try {
      sink?.toolCallFail?.({
        id,
  ...
  if (request.method !== "tools/call") {
    const errorResp = {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Invalid method for tools/call handler" },
    } as MCPResponse;
    try {
      sink?.toolCallFail?.({
        id,
  ...
  try {
    sink?.toolCallStart?.({ id, name: toolName });
  } catch { /* ignore sink errors */ }
  ...
  try {
      ...
      case "search":
        {
          ...
          if (!v.success) {
            const errorResp = {
              jsonrpc: "2.0",
              id,
              error: { code: -32602, message: v.error.message },
            } as MCPResponse;
            try {
              sink?.toolCallFail?.({
                id,
    ...
    if ("error" in result && result.error) {
      ...
      try {
        sink?.toolCallSuccess?.({ id, name: toolName });
      } catch { /* ignore sink errors */ }
    }
