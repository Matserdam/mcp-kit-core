 ...
 * console.log('Authenticated user:', result.middleware.user); // eslint-disable-line no-console
 * ```
 */
export async function executeHTTPAuth<TAuth>(
  ...
  if (!request) {
    throw new MCPAuthError(
      "HTTP request required for HTTP auth middleware",
    ...
    if (auth.validateTokenWithSecurity) {
      ...
      if (auth.requiredScopes && auth.requiredScopes.length > 0) {
        ...
        if (missingScopes.length > 0) {
          throw new MCPAuthError(
            `Insufficient scope. Required: ${auth.requiredScopes.join(", ")}. Provided: ${
              tokenScopes.join(", ")
            }`,
      ...
      if (securityResult.aud && securityResult.aud.length > 0) {
        ...
        if (
          !securityResult.aud.includes(resourceDomain) && !securityResult.aud.includes(resourceUri)
        ) {
          throw new MCPAuthError(
            `Invalid audience. Required: ${
              securityResult.aud.join(", ")
 ...
 * console.log('Authenticated user:', result.middleware.userId); // eslint-disable-line no-console
 * ```
 */
export async function executeSTDIOAuth<TAuth>(
  ...
  if (!env) {
    throw new MCPAuthError(
      "Environment required for STDIO auth middleware",
      MCP_AUTH_ERROR_CODES.BAD_REQUEST,
    );
  }
...
export async function executeAuth<TAuth>(
  ...
  switch (auth.type) {
    case "http":
      ...
      return await executeHTTPAuth(request, auth, resourceUriExtractor);
    ...
    case "stdio":
      ...
      return await executeSTDIOAuth(env, auth);
