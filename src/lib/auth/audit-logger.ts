...
export class MCPAuthAuditLogger {
  ...
  constructor(config: MCPAuthAuditConfig = {}) {
    this.config = {
      ...
      correlationIdGenerator: config.correlationIdGenerator ??
        this.defaultCorrelationIdGenerator.bind(this),
  ...
  private extractRequestInfo(request: MCPRequestWithHeaders): MCPAuthAuditEntry["requestInfo"] {
    ...
    return {
      ...
      resourceUri: String(
        (request.params as Record<string, unknown>)?.uri ||
...
export const auditLoggers = {
  ...
  development: new MCPAuthAuditLogger({
    ...
    logger: (entry) => {
      ...
      debugLoggers.audit(
        `${color}[${entry.severity.toUpperCase()}]\x1b[0m ${JSON.stringify(entry, null, 2)}`,
      );
    },
  }),
