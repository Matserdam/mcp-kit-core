...
export async function handleFetchRequest(opts: {
  ...
  const isClientResponse = rpc && typeof rpc === "object" &&
    (rpc as Record<string, unknown>).jsonrpc === "2.0" && !("method" in rpc) &&
    (("result" in (rpc as Record<string, unknown>)) ||
      ("error" in (rpc as Record<string, unknown>)));
  if (isClientResponse) {
  ...
  if ("error" in parsed) {
    const errorResponse = responseJson({ jsonrpc: "2.0", id: parsed.id, error: parsed.error }, {
      status: 400,
    });
    return defaultCORSHandler.addCORSHeaders(errorResponse, request);
  }
  ...
  const isNotification = rpc && typeof rpc === "object" &&
    (rpc as Record<string, unknown>).jsonrpc === "2.0" &&
    typeof (rpc as Record<string, unknown>).method === "string" && !("id" in rpc);
  if (isNotification) {
    void handleRPC(parsed, toolkits);
    const response = new Response(null, { status: 202 });
    return defaultCORSHandler.addCORSHeaders(response, request);
  }

  const response = await handleRPC(parsed, toolkits, {
    httpRequest: request,
    discovery,
    eventSink,
  });
  ...
  try {
    eventSink?.rpcSucceeded?.({
      id: response.id,
      method: (parsed as { method?: string }).method ?? "unknown",
    });
