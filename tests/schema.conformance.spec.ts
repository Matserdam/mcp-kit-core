...
const zContentImage = z.object({
  type: z.literal("image"),
  data: z.string(),
  mimeType: z.string(),
});
const zContentAudio = z.object({
  type: z.literal("audio"),
  data: z.string(),
  mimeType: z.string(),
});
const zContentResourceLink = z.object({
...
const zCallToolResult = z.object({
  content: z.array(
    z.union([zContentText, zContentImage, zContentAudio, zContentResourceLink, zContentResource]),
  ),
...
describe("Schema conformance (MCP 2025-06-18)", () => {
  ...
  it("tools/list matches schema", async () => {
    ...
    const res = await handleRPC(req, [
      { namespace: "demo", tools: [] } as MCPToolkit<unknown, unknown>,
    ]);
    expect(zJSONRPCResponse.parse(res)).toBeTruthy();
    const response = res as { result: unknown };
  ...
  it("tools/call returns ContentBlock[]", async () => {
    ...
    const res = await handleRPC(req, [
      { namespace: "demo", tools: [tool] } as MCPToolkit<unknown, unknown>,
    ]);
    expect(zJSONRPCResponse.parse(res)).toBeTruthy();
    const response = res as { result: unknown };
    ...
    const res = await handleRPC(req, [
      {
        ...
        resources: [{
          uri: "file:///tmp/a.txt",
          name: "a",
          read: () => ({ contents: [{ uri: "file:///tmp/a.txt", text: "x" }] }),
        }],
      } as MCPToolkit<unknown, unknown>,
    ]);
    expect(zJSONRPCResponse.parse(res)).toBeTruthy();
    const response = res as { result: unknown };
  ...
  it("resources/read matches schema", async () => {
    const tk: MCPToolkit<unknown, unknown> = {
      namespace: "demo",
      resources: [{
        uri: "file:///tmp/a.txt",
        name: "a",
        read: () => ({ contents: [{ uri: "file:///tmp/a.txt", text: "hello" }] }),
      }],
    } as MCPToolkit<unknown, unknown>;
    const req: MCPRequest = {
      id: 6,
