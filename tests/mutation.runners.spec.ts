...
import type { MCPResourceProvider, MCPToolkit } from "../src/types/toolkit";
import type {
  ContentResource,
  MCPToolCallResult,
  MCPToolsCallParams,
...
const createMockToolkit = (
  overrides: Partial<MCPToolkit<unknown, unknown>> = {},
): MCPToolkit<unknown, unknown> => ({
  namespace: "test",
  ...overrides,
});
...
const createMockResource = (
  ...
  title?: string,
  description?: string,
): MCPResourceProvider<Record<string, unknown>> => ({
...
describe("Mutation Tests - Fetch Runner", () => {
  ...
  describe("URI Resolution Edge Cases", () => {
    it("should handle id as http URL without explicit uri", async () => {
      const params: MCPToolsCallParams = {
        name: "fetch",
        arguments: { id: "https://example.com/resource" },
      };
      const result = await runFetch("test-id", params, []);
      const toolCallResult = result.result as MCPToolCallResult;
      expect(toolCallResult).toBeDefined();
  ...
  describe("Provider Resolution Edge Cases", () => {
    it("should handle provider read throwing error", async () => {
      ...
      const params: MCPToolsCallParams = {
        name: "fetch",
        arguments: { id: "test-id", uri: "test://resource" },
      };
  ...
  describe("Template Resolution Edge Cases", () => {
    it("should handle template read throwing error", async () => {
      ...
      const params: MCPToolsCallParams = {
        name: "fetch",
        arguments: { id: "test-id", uri: "test://123/resource" },
      };
      const result = await runFetch("test-id", params, [toolkit]);
    ...
    it("should handle text content", async () => {
      const mockProvider = {
        ...
        read: async () => {
          ...
          return {
            contents: [{
              uri: "test://resource" as ResourceUri,
              text: "test content",
              name: "test",
              mimeType: "text/plain",
            }],
      ...
      expect("text" in resourceContent.resource && resourceContent.resource.text).toBe(
        "test content",
      );
    });
      ...
      const mockProvider = {
        ...
        read: async () => {
          ...
          return {
            contents: [{
              uri: "test://resource" as ResourceUri,
              blob: btoa(String.fromCharCode(...Array.from(new Uint8Array([1, 2, 3])))),
              name: "test",
              mimeType: "application/octet-stream",
            }],
          };
  ...
  describe("Empty Toolkit Edge Cases", () => {
    it("should handle empty toolkits array", async () => {
      const params: MCPToolsCallParams = {
        name: "fetch",
        arguments: { id: "test-id", uri: "https://example.com/resource" },
      };
      const result = await runFetch("test-id", params, []);
      const toolCallResult = result.result as MCPToolCallResult;
      expect(toolCallResult).toBeDefined();
