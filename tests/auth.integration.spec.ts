...
const createPublicToolkit = (namespace: string): MCPToolkit<unknown, unknown> => ({
  ...
  resourceTemplates: [{
    ...
    read: (uri: string) => ({
      contents: [{ uri: uri as `${string}://${string}`, text: "public template content" }],
    }),
  ...
  prompts: [{
    ...
    messages: () =>
      Promise.resolve([{
        role: "user",
        content: [{ type: "text", text: "public prompt message" }],
      }]),
...
const createPrivateToolkit = (namespace: string): MCPToolkit<unknown, { user: string }> => {
  ...
  return {
    ...
    resourceTemplates: [{
      ...
      read: (uri: string) => ({
        contents: [{ uri: uri as `${string}://${string}`, text: "private template content" }],
      }),
    } as MCPResourceTemplateProvider<unknown>],
    prompts: [{
      name: "private-prompt",
      ...
      messages: () =>
        Promise.resolve([{
          role: "user",
          content: [{ type: "text", text: "private prompt message" }],
        }]),
    } as unknown as MCPPromptDef<unknown, unknown>],
  };
};
...
describe("Authentication Integration Tests", () => {
  ...
  describe("Tools List Authentication", () => {
    it("should include all toolkit tools when no auth provided", async () => {
      ...
      const result =
        (response as MCPResponse & { result: { tools: Array<{ name: string }> } }).result;
  ...
  describe("Tool Call Authentication", () => {
    it("should allow access to public toolkit tool without auth", async () => {
      ...
      const result =
        (response as MCPResponse & { result: { content: Array<{ text: string }> } }).result;
  ...
  describe("Resources List Authentication", () => {
    it("should include all toolkit resources when no auth provided", async () => {
      ...
      expect(response.jsonrpc).toBe("2.0");
      const result =
        (response as MCPResponse & { result: { resources: Array<{ uri: string }> } }).result;
  ...
  describe("Resources Read Authentication", () => {
    it("should allow access to public toolkit resource without auth", async () => {
      ...
      expect("error" in response).toBe(false);
      const result =
        (response as MCPResponse & { result: { contents: Array<{ text: string }> } }).result;
  ...
  describe("Resource Templates List Authentication", () => {
    it("should include all toolkit templates when no auth provided", async () => {
      ...
      expect(response.jsonrpc).toBe("2.0");
      const result =
        (response as MCPResponse & { result: { resourceTemplates: Array<{ name: string }> } })
          .result;
  ...
  describe("Prompts List Authentication", () => {
    it("should include all toolkit prompts when no auth provided", async () => {
      ...
      expect(response.jsonrpc).toBe("2.0");
      const result =
        (response as MCPResponse & { result: { prompts: Array<{ name: string }> } }).result;
  ...
  describe("Prompts Get Authentication", () => {
    it("should allow access to public toolkit prompt without auth", async () => {
      ...
      const result = (response as MCPResponse & {
        result: { messages: Array<{ content: Array<{ text: string }> }> };
      }).result;
  ...
  describe("Canonical Tools Authentication", () => {
    ...
    it("should allow fetch of private resource when auth provided", async () => {
      ...
      expect("error" in response).toBe(false);
      const result =
        (response as MCPResponse & { result: { content: Array<{ resource: { text: string } }> } })
          .result;
