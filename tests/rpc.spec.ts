...
describe("handleRPC", () => {
  const makeToolkit = (tools: MCPTool<unknown, unknown>[]): MCPToolkit<unknown, unknown> => ({
    namespace: "ns",
    tools,
  });
  ...
  it("returns initialize payload", async () => {
    ...
    const response = res as MCPResponse & {
      result: { protocolVersion: unknown; capabilities: { tools: { listChanged: boolean } } };
    };
  ...
  it("calls a tool and returns MCPToolCallResult", async () => {
    const tool: MCPTool<unknown, unknown> = {
      ...
      run: (args: Record<string, unknown>) => ({
        content: [{
          type: "text",
          text: String((args?.a as number ?? 0) + (args?.b as number ?? 0)),
        }],
      }),
    ...
    const req: MCPRequest = {
      id: 2,
      method: "tools/call",
      params: { name: "ns_sum", arguments: { a: 1, b: 2 } },
    } as MCPRequest;
    const res = await handleRPC(req, [makeToolkit([tool])]);
  ...
  it("errors when toolkit not found", async () => {
    const req: MCPRequest = {
      id: 3,
      method: "tools/call",
      params: { name: "missing_sum", arguments: {} },
    } as MCPRequest;
    const res = await handleRPC(req, []);
  ...
  it("errors when tool not found", async () => {
    const req: MCPRequest = {
      id: 4,
      method: "tools/call",
      params: { name: "ns_missing", arguments: {} },
    } as MCPRequest;
  ...
  it("validates zod and errors on invalid args", async () => {
    ...
    const req: MCPRequest = {
      id: 5,
      method: "tools/call",
      params: { name: "ns_needsA", arguments: { a: 1 } },
    } as MCPRequest;
    const res = await handleRPC(req, [makeToolkit([tool])]);
