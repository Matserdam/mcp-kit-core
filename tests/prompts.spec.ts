...
describe('MCP prompts', () => {
  it('prompts/list returns toolkit prompts with namespaced names', async () => {
      ...
      toolkits: [
        {
          namespace: 'pokemon',
          prompts: [
            { name: 'appearance', title: 'Pokémon appearance', description: 'appearance prompt', input: { zod: z.object({ name: z.string() }) }, messages: (input: { name: string }) => [{ role: 'user', content: { type: 'text', text: `What is the appearance of ${input.name}?` } }] },
          ],
        },
      ],
    });
    ...
    const response = resp as { result: { prompts: Array<{ name: string; title: string; arguments: unknown[] }> } };
  ...
  it('prompts/get returns a prompt with messages array per protocol', async () => {
    const prompt: MCPPromptDef =
      { name: 'appearance', title: 'Pokémon appearance', description: 'appearance prompt', arguments: [{ name: 'name', description: 'name of the pokemon', required: true }], messages: (input: { name: string }) => [{ role: 'user', content: { type: 'text', text: `What is the appearance of ${input.name}?` } }] };
