...
describe("Discovery Handler", () => {
  ...
  describe("WWW-Authenticate Header Generation", () => {
    it("should create proper WWW-Authenticate header", () => {
      const header = discoveryHandler.createWWWAuthenticateHeader(
      ...
      expect(header).toContain(
        'discovery_uri="https://mcp.example.com/.well-known/oauth-authorization-server"',
      );
    });
    ...
    it("should handle invalid URLs gracefully", () => {
      ...
      expect(header).toContain('Bearer realm="mcp-server"');
      expect(header).toContain(
        'discovery_uri="https://mcp-server/.well-known/oauth-authorization-server"',
      );
    });
  });
  ...
  describe("Configuration Validation", () => {
    ...
    it("should detect missing required fields", () => {
      ...
      expect(validation.errors).toContain(
        "Authorization server authorization endpoint is required",
      );
      expect(validation.errors).toContain("Authorization server token endpoint is required");
      expect(validation.errors).toContain(
        "Authorization server must support at least one response type",
      );
      expect(validation.errors).toContain(
      ...
      expect(validation.errors).toContain(
        "Protected resource must have at least one authorization server",
      );
    });
  });
  ...
  describe("Enhanced Error Responses", () => {
    it("should include discovery information in 401 responses", async () => {
      ...
      const errorResponse = await response.json() as {
        error: {
          data: {
            discovery_endpoints: { authorization_server: string; protected_resource: string };
          };
        };
      };
      expect(errorResponse.error.data.discovery_endpoints.authorization_server).toBe(
        "https://mcp.example.com/.well-known/oauth-authorization-server",
      );
      expect(errorResponse.error.data.discovery_endpoints.protected_resource).toBe(
