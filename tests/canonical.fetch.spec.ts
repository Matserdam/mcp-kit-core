...
describe("canonical fetch resolves via providers/templates (async)", () => {
  const makeToolkit = (
    partial: Partial<MCPToolkit<unknown, unknown>>,
  ): MCPToolkit<
    unknown,
    unknown
  > => ({ namespace: "ns", ...partial } as MCPToolkit<unknown, unknown>);
  ...
  it("resolves via async resource provider", async () => {
    const tk = makeToolkit({
      resources: [{
        ...
        name: "doc",
        read: () => ({
          contents: [{ uri: "custom://doc" as ResourceUri, text: "ok", mimeType: "text/plain" }],
        }),
    ...
    const req: MCPRequest = {
      id: 1,
      method: "tools/call",
      params: { name: "fetch", arguments: { id: "doc", uri: "custom://doc" } },
    } as MCPRequest;
  ...
  it("resolves via async resource template", async () => {
    const tk = makeToolkit({
      resourceTemplates: [{
        descriptor: {
          uriTemplate: "notes://{name}",
          name: "notes",
          title: "Notes",
          description: "Notes template",
          mimeType: "text/plain",
        },
        read: (uri: ResourceUri) => ({
    ...
    const req: MCPRequest = {
      id: 2,
      method: "tools/call",
      params: { name: "fetch", arguments: { id: "n", uri: "notes://readme" } },
    } as MCPRequest;
    const res = await handleRPC(req, [tk]);
    const response = res as MCPResponse & {
      result: { content: Array<{ type: string; resource: { text: string } }> };
